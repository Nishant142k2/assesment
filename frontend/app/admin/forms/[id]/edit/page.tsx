...
export default function EditFormPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await updateForm(id, { title: title.trim(), description: description.trim(), fields });
      router.push("/admin");
    } catch {
      setError("Failed to save. Check that your FastAPI server is running.");
      setSaving(false);
    }
  };
  ...
  if (notFound) return (
    <div style={{ padding: 40, textAlign: "center" }}>
      Form not found. <Link href="/admin" style={{ color: "#2563eb" }}>Go back</Link>
    </div>
  );
  if (!form) return <div style={{ padding: 40, textAlign: "center", color: "#9ca3af" }}>Loading...</div>;
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 32px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 15 }}>Edit Form</span>
          </div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={title} onChange={(e) => { setTitle(e.target.value); setError(""); }} style={inputStyle} placeholder="Form title" />
          </div>
          <div>
            <label style={labelStyle}>Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} style={{ ...inputStyle, resize: "vertical" }} placeholder="Brief description" />
          </div>
        </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 24 }}>
          ...
          {fields.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 24px", color: "#9ca3af", border: "2px dashed #e5e7eb", borderRadius: 8 }}>
              <p style={{ marginBottom: 12 }}>No fields yet.</p>
